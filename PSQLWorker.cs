...
namespace CreatioManagmentTools
{
    internal class PSQLWorker
    {
        ...
        public void RestoreDb(string projectName, decimal redisDb)
        {
            ...
            var CommandText = $"CREATE DATABASE {projectName} WITH OWNER = {settings.PSQLUserID} ENCODING = 'UTF8' CONNECTION LIMIT = -1";
        ...
        private void PostgreSqlRestore(string inputFile, string projectName)
        {
            string dumpCommand = $"set PGPASSWORD={settings.PSQLPassword}\n" +
                                 $"\"{settings.PSQLPath}\\pg_restore.exe\" --host {settings.PSQLServer} --port {settings.PSQLPort} --username={settings.PSQLUserID} --dbname={projectName} --no-owner --no-privileges {inputFile} && exit";
